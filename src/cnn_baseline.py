...
class CNN:
  ...
  def train(self):
    ...
    history = History()
    file_name = str(reg) + "_" + str(dropout) + ".hdf5"
    checkpointer = ModelCheckpoint(filepath=file_name, save_best_only=True, mode='auto', verbose=1, monitor="val_loss")
    model.fit_generator(datagen.flow(X_train, Y_train, batch_size=batch_size),
              samples_per_epoch=X_train.shape[0],
              nb_epoch=nb_epoch, show_accuracy=True,
              validation_data=(X_val, Y_val),
              nb_worker=1, callbacks=[history, checkpointer], verbose=2)
    ...
    if save_weights:
      weights_path = out_location + str(final_acc) + '_final_weights.h5'
